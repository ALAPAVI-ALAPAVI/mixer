...
export async function saveTrackOffline(track) {
  ...
  return saveTrackOfflineFromBlob(track.id, blob, { title: track.title, artist: track.artist });
}

// Same as saveTrackOffline, but for when the audio bytes are already sitting
// in memory (e.g. right after this device uploaded the file) — skips the
// network fetch entirely, so the device that uploaded a song can always play
// it back offline without needing to "download" what it already has.
export async function saveTrackOfflineFromBlob(trackId, blob, { title, artist } = {}) {
