...
export async function insertTrack({ userId, title, artist, blobUrl, sizeBytes, contentHash }) {
  await ensureSchema();
  const rows = await sql`
    INSERT INTO tracks (user_id, title, artist, blob_url, size_bytes, content_hash)
    VALUES (${userId}, ${title}, ${artist || null}, ${blobUrl}, ${sizeBytes || null}, ${contentHash || null})
    RETURNING id, title, artist, blob_url, size_bytes, content_hash, created_at;
  `;
  return rows[0];
}
