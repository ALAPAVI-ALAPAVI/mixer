...
export async function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    `;
    // Safe to run even if the table already existed before this column did.
    await sql`ALTER TABLE tracks ADD COLUMN IF NOT EXISTS content_hash VARCHAR(64);`;
    await sql`ALTER TABLE tracks ADD COLUMN IF NOT EXISTS play_count INTEGER NOT NULL DEFAULT 0;`;
    await sql`ALTER TABLE tracks ADD COLUMN IF NOT EXISTS duration_seconds REAL;`;
    await sql`
...
export async function listTracksForUser(userId) {
  await ensureSchema();
  return sql`SELECT id, title, artist, blob_url, size_bytes, content_hash, created_at
             FROM tracks WHERE user_id = ${userId} ORDER BY created_at DESC`;
}

export async function insertTrack({ userId, title, artist, blobUrl, sizeBytes, contentHash, durationSeconds }) {
  await ensureSchema();
  const rows = await sql`
    INSERT INTO tracks (user_id, title, artist, blob_url, size_bytes, content_hash, duration_seconds)
    VALUES (${userId}, ${title}, ${artist || null}, ${blobUrl}, ${sizeBytes || null}, ${contentHash || null}, ${durationSeconds || null})
    RETURNING id, title, artist, blob_url, size_bytes, content_hash, duration_seconds, created_at;
  `;
  return rows[0];
}

// Used to reject uploading the exact same audio file twice for one account.
export async function findTrackByHash(userId, contentHash) {
  await ensureSchema();
  if (!contentHash) return null;
  const rows = await sql`
    SELECT id FROM tracks WHERE user_id = ${userId} AND content_hash = ${contentHash} LIMIT 1
  `;
  return rows[0] || null;
}
...
export async function renameFolder(folderId, userId, name) {
  ...
  const rows = await sql`
    ...
    WHERE id = ${folderId} AND user_id = ${userId} AND is_default = false
    RETURNING id, name, is_default, created_at;
  `;
  return rows[0] || null;
}
...
export async function incrementPlayCount(trackId, userId) {
  await ensureSchema();
  await sql`
...
export async function getAccountStats(userId) {
  ...
  const mostPlayedRows = await sql`
    SELECT id, title, artist, play_count
    FROM tracks
    WHERE user_id = ${userId} AND play_count > 0
    ORDER BY play_count DESC, created_at ASC
    LIMIT 1
  `;

  return {
    totalSongs: totals[0]?.total_songs || 0,
    totalDurationSeconds: Number(totals[0]?.total_duration_seconds) || 0,
